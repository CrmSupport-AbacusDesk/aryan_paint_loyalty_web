...
export class DealerDetailComponent implements OnInit {
  ...
  myPointList(){
      ...
      this.db.post_rqst( {'filter': this.filter, 'contractor_id':this.contractor_id}, 'offer/getReferralData').subscribe( r =>
          {
              this.loading_list = false;
              this.filter.mode = 1;
              console.log(r);
              this.myPoint = r.coupon;
...
getAssignpainter(){
    this.filter.karigar_id = this.karigar_id;
    this.db.post_rqst({'filter': this.filter },'karigar/sales_user_painter')
    .subscribe(d=>{
        console.log(d);
        this.karigars = d.karigars;
        this.painterCount=d.karigars_count;
